...
| 2 | **Validation Workflow:** which verification steps should plans prefer? | Unit tests (`uv run pytest`), lint and format (`ruff`), frontend type check and build (npm), and manual browser QA for any UI-visible change. Integration tests once an API surface exists. |
